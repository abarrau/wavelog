...
function echo_table_header_col($ctx, $name) {
    switch($name) {
        case 'Mode': echo '<th>'.__t('Mode').'</th>'; break;
        case 'RSTS': echo '<th class="d-none d-sm-table-cell">'.__t('RST (S)').'</th>'; break;
        case 'RSTR': echo '<th class="d-none d-sm-table-cell">'.__t('RST (R)').'</th>'; break;
        case 'Country': echo '<th>'.__t('Country').'</th>'; break;
        case 'IOTA': echo '<th>'.__t('IOTA Reference').'</th>'; break;
        case 'SOTA': echo '<th>'.__t('SOTA Reference').'</th>'; break;
        case 'WWFF': echo '<th>'.__t('WWFF Reference').'</th>'; break;
        case 'POTA': echo '<th>'.__t('POTA Reference').'</th>'; break;
        case 'State': echo '<th>'.__t('State').'</th>'; break;
        case 'Grid': echo '<th>'.__t('Gridsquare').'</th>'; break;
        case 'Distance': echo '<th>'.__t('Distance').'</th>'; break;
        case 'Band': echo '<th>'.__t('Band').'</th>'; break;
        case 'Frequency': echo '<th>'.__t('Frequency').'</th>'; break;
        case 'Operator': echo '<th>'.__t('Operator').'</th>'; break;
        case 'Name': echo '<th>'.__t('Name').'</th>'; break;
    }
}
...
<?php if(($this->config->item('use_auth') && ($this->session->userdata('user_type') >= 2)) || $this->config->item('use_auth') === FALSE) { ?>
    ...
    <?php if (version_compare(PHP_VERSION, '7.4.0') <= 0) { ?>
        ...
        <?php echo __t('You need to upgrade your PHP version. Minimum version is 7.4. Your version is') . ' ' . PHP_VERSION . '.';?>
        </div>
    <?php } ?>

    <?php if ($countryCount == 0) { ?>
        <div class="alert alert-danger mt-3" role="alert">
        <?php echo __t('dashboard_country_files_warning'); ?>
        </div>
    <?php } ?>

    <?php if ($locationCount == 0) { ?>
        <div class="alert alert-danger" role="alert">
        <?php echo __t('dashboard_locations_warning'); ?>
        </div>
    <?php } ?>

    <?php if ($logbookCount == 0) { ?>
        <div class="alert alert-danger" role="alert">
        <?php echo __t('dashboard_logbooks_warning'); ?>
        </div>
    <?php } ?>
    ...
    <?php if($todays_qsos >= 1) { ?>
        <div class="alert alert-success" role="alert" style="margin-top: 1rem;">
              <?php printf(ngettext('You have had %d QSO Today!', 'You have had %d QSOs Today!', $todays_qsos), $todays_qsos); ?>
        </div>
    <?php } else { ?>
        <div class="alert alert-warning" role="alert" style="margin-top: 1rem;">
              <span class="badge text-bg-info"><?php echo __t('Important'); ?></span> <i class="fas fa-broadcast-tower"></i> <?php echo __t('You have made no QSOs today; time to turn on the radio!'); ?>
        </div>
    <?php } ?>
    <?php } ?>
    ...
    <?php if($current_active == 0) { ?>
        <div class="alert alert-danger" role="alert">
          <?php echo __t('Attention: you need to set an active station location.'); ?>
        </div>
    <?php } ?>
    ...
    <?php if ($this->session->userdata('user_id')) { ?>
        <?php
            ...
            <div class="alert alert-danger" role="alert">
                <span class="badge text-bg-info"><?php echo __t('Important'); ?></span> <i class="fas fa-hourglass-end"></i> <?php echo __t('At least one of your LoTW certificates is expired!'); ?>
            </div>
        <?php } ?>

        <?php if($this->LotwCert->lotw_cert_expiring($this->session->userdata('user_id'), $current_date) == true) { ?>
            <div class="alert alert-warning" role="alert">
                <span class="badge text-bg-info"><?php echo __t('Important'); ?></span> <i class="fas fa-hourglass-half"></i> <?php echo __t('At least one of your LoTW certificates is about to expire!'); ?>
            </div>
        <?php } ?>
    <?php } ?>
  ...
  <div class="col-sm-8">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover border-top">
            ...
            <thead>
                <tr class="titles">
                    <th><?php echo __t('Date'); ?></th>

                    <?php if(($this->config->item('use_auth') && ($this->session->userdata('user_type') >= 2)) || $this->config->item('use_auth') === FALSE || ($this->config->item('show_time'))) { ?>
                    <th><?php echo __t('Time'); ?></th>
                    <?php } ?>
                    <th><?php echo __t('Call'); ?></th>
                    <?php
  ...
  <div class="col-sm-4">
    ...
    <div class="table-responsive">
        ...
        <table class="table table-striped border-top">
            <tr class="titles">
                <td colspan="2"><i class="fas fa-chart-bar"></i> <?php echo __t('QSOs Breakdown'); ?></td>
            </tr>

            <tr>
                <td width="50%"><?php echo __t('Total'); ?></td>
                <td width="50%"><?php echo $total_qsos; ?></td>
            </tr>

            <tr>
                <td width="50%"><?php echo __t('Year'); ?></td>
                <td width="50%"><?php echo $year_qsos; ?></td>
            </tr>

            <tr>
                <td width="50%"><?php echo __t('Month'); ?></td>
        ...
        <table class="table table-striped border-top">
            <tr class="titles">
                <td colspan="2"><i class="fas fa-globe-europe"></i> <?php echo __t('Countries Breakdown'); ?></td>
            </tr>

            <tr>
                <td width="50%"><?php echo __t('Worked'); ?></td>
                <td width="50%"><?php echo $total_countries; ?></td>
            </tr>
            <tr>
                <td width="50%"><a href="#" onclick="return false" title="QSL Cards / eQSL / LoTW" data-bs-toggle="tooltip"><?php echo __t('Confirmed'); ?></a></td>
                <td width="50%">
            ...
            <tr>
                <td width="50%"><?php echo __t('Needed'); ?></td>
        ...
        <table class="table table-striped border-top">
            <tr class="titles">
                <td colspan="2"><i class="fas fa-envelope"></i> <?php echo __t('QSL Cards'); ?></td>
                <td colspan="1"><?php echo __t('Today'); ?></td>
            </tr>

            <tr>
                <td width="50%"><?php echo __t('Sent'); ?></td>
            ...
            <tr>
                <td width="50%"><?php echo __t('Received'); ?></td>
            ...
            <tr>
                <td width="50%"><?php echo __t('Requested'); ?></td>
        ...
        <table class="table table-striped border-top">
            <tr class="titles">
                <td colspan="2"><i class="fas fa-address-card"></i> <?php echo __t('eQSL Cards'); ?></td>
                <td colspan="1"><?php echo __t('Today'); ?></td>
            </tr>
        ...
        <table class="table table-striped border-top">
            <tr class="titles">
                <td colspan="2"><i class="fas fa-list"></i> <?php echo __t('Logbook of the World'); ?></td>
                <td colspan="1"><?php echo __t('Today'); ?></td>
            </tr>
        ...
        <tr>
            <td width="50%"><?php echo __t('Confirmed'); ?></td>
