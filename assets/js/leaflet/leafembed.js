...
var qsoIcon = L.divIcon({className:'cspot_qso cspot'}); //default (fas fa-dot-circle red)
var qsoconfirmIcon = L.divIcon({className:'cspot_qsoconfirm cspot'});
	...
	if (_url_qso != '') {
		...
		if (_visitor) {
			...
			$.ajax({
				...
				error: function() { askForPlots(_url_qso, options); console.log('[ERROR][initplot()] ajax get_map_custom() function return error.'); },
    ...
    $.ajax({
        ...
        success: function(plotjson) {
        	if ((typeof plotjson['markers'] !== "undefined")&&(plotjson['markers'].length>0)) {
				for (i=0;i<plotjson['markers'].length;i++) { createPlots(plotjson['markers'][i], options); }
        	}
...
function createPlots(_plot, options={}) {
	...
	// add qso id to plot //
	if ((typeof _plot.qsoid !== "undefined") && (_plot.qsoid > 0)) {
		$(options.map_id+' .cspot:last-child').attr('data-qsoid',_plot.qsoid);
	}
...
// HighLight plot on MAP on hover QSO log //
function setSyncLogWithMap(mapId='map', logTabClass='log-sync-map') {
	$.ajax({
		url: base_url+'index.php/user_options/get_map_custom', type: 'GET', dataType: 'json',
		error: function() { console.log('[ERROR][setSyncLogWithMap()] ajax get_map_custom() function return error.'); },
		success: function(json_mapinfo) {
			if ((typeof json_mapinfo.highlightqso_show !== "undefined") && (json_mapinfo.highlightqso_show=="1")) {
				$('.'+logTabClass+' .trqso').off('mouseenter').on('mouseenter', function () {
					$('#'+mapId+' .cspot[data-qsoid="'+$(this).attr('id').replace('qso_','')+'"]').click();
				});
				$('.'+logTabClass+' .trqso').off('mouseleave').on('mouseleave', function () {
					$('#'+mapId+' .cspot[data-qsoid="'+$(this).attr('id').replace('qso_','')+'"]').click();
				});
			}
		}
